<div class="p-4 sm:p-6 bg-white rounded-xl shadow space-y-5">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Academic Subjects</h1>
            <p class="text-slate-500 text-sm mt-1">Manage your school's subjects and assign teachers to each.</p>
        </div>
            ...
            <table class="w-full min-w-[600px] text-sm">
                <thead class="bg-[#155E8A] text-white">
                    <tr>
                        ...
                        <th class="px-4 sm:px-6 py-3 text-left text-xs font-semibold uppercase">Teachers</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($subjects as $subject)
                        <tr class="hover:bg-sky-50/60 transition-colors">
                            <td class="px-4 sm:px-6 py-3 font-semibold text-slate-800">{{ $subject->name }}</td>
                            <td class="px-4 sm:px-6 py-3 text-slate-500 capitalize">{{ $subject->level }}</td>
                            <td class="px-4 sm:px-6 py-3 text-slate-500">{{ $subject->teachers()->count() }} assigned
                            </td>
                            <td class="px-4 sm:px-6 py-3">
                                <div class="flex justify-center flex-wrap gap-2">
                                    <button wire:click="openEditModal({{ $subject->id }})" wire:loading.attr="disabled"
                                        wire:target="openEditModal({{ $subject->id }})"
                                    ...
                                    <button wire:click="confirmDelete({{ $subject->id }})" wire:loading.attr="disabled"
                                        wire:target="confirmDelete({{ $subject->id }})"
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="text-center py-14">
                                ...
                                <p class="text-slate-500 text-sm">No subjects yet. Create your first one to get started.
                                </p>
                            </td>
                        </tr>
                    @endforelse
                ...
                <div class="relative bg-[#155E8A] px-5 sm:px-6 py-5 shrink-0">
                    ...
                    <div class="flex items-start justify-between gap-3">
                        @include('partials.modal-school-header', [
                            'title' => ($editingId ? 'Edit' : 'New') . ' Subject',
                        ])
                ...
                <form wire:submit.prevent="save" class="flex-1 overflow-y-auto bg-[#F8FAFC] p-6 space-y-5">
                    ...
                    <div class="rounded-xl border-2 border-sky-100 bg-sky-50/40 overflow-hidden">
                        ...
                        <div class="flex items-start gap-3 px-5 py-4 border-b border-sky-100 bg-sky-50">
                            ...
                            <div>
                                <h3 class="text-sm font-bold text-[#155E8A]">Assign Teachers</h3>
                                <p class="text-xs text-slate-500 mt-0.5">Select one or more teachers responsible for
                                    this subject.</p>
                            </div>
                        </div>

                        <div class="p-5">
                            @if ($teacherOptions->count())
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    @foreach ($teacherOptions as $teacher)
                                        <label
                                            ...
                                            <input type="checkbox" value="{{ $teacher->id }}" wire:model="teacherIds"
        ...
        <div class="fixed inset-0 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-3 z-50">
            <div class="bg-white w-full max-w-sm rounded-xl shadow-xl border border-red-100">
                <div
                    ...
                    <h3 class="text-sm font-semibold text-white">Delete Subject</h3>
                ...
                <div class="p-4 text-sm text-slate-700 space-y-2">
                    <p>Delete <strong>{{ $deletingName }}</strong>? This cannot be undone.</p>
                    <p class="text-xs text-red-700 bg-red-50 border border-red-200 rounded-lg px-3 py-2">
                        <i class="fas fa-triangle-exclamation mr-1"></i>
                        This also permanently deletes every student grade and grade-audit record filed under this
                        subject.
                    </p>
                </div>
